...
var Town = {
	...
	tasks: [
		{Heal: NPC.Akara, 	Shop: NPC.Akara,   Gamble: NPC.Gheed, 	Repair: NPC.Charsi, Merc: NPC.Kashya, 	 Key: NPC.Akara, 	CainID: NPC.Cain, Pots: NPC.Akara},
		{Heal: NPC.Fara, 	Shop: NPC.Drognan, Gamble: NPC.Elzix, 	Repair: NPC.Fara, 	Merc: NPC.Greiz, 	 Key: NPC.Lysander, CainID: NPC.Cain, Pots: NPC.Lysander},
		{Heal: NPC.Ormus, 	Shop: NPC.Ormus,   Gamble: NPC.Alkor, 	Repair: NPC.Hratli, Merc: NPC.Asheara, 	 Key: NPC.Hratli,	CainID: NPC.Cain, Pots: NPC.Alkor},
		{Heal: NPC.Jamella, Shop: NPC.Jamella, Gamble: NPC.Jamella, Repair: NPC.Halbu, 	Merc: NPC.Tyrael, 	 Key: NPC.Jamella,	CainID: NPC.Cain, Pots: NPC.Jamella},
		{Heal: NPC.Malah, 	Shop: NPC.Malah,   Gamble: NPC.Anya, 	Repair: NPC.Larzuk, Merc: NPC.Qual_Kehk, Key: NPC.Malah,	CainID: NPC.Cain, Pots: NPC.Malah}
	],
	...
	doChores: function (repair = false) {
		...
		this.buyPotions();
		this.shopItems();
		if (me.getStat(12) >= 3) this.repair(repair);
		this.gamble();
		this.reviveMerc();
		Cubing.doCubing();
	...
	stackPotions: function(type, num=5) {
		if (me.gold < 5000) return false;
		var npc, i, pot, pre;
		...
		switch (type) {
			case 513:
			case "stamina":
				print("Stacking " + num + " stamina potions");
				break;
			case 514:
			case "antidote":
				print("Stacking " + num + " antidote potions");
				break;
			case 517:
			case "thawing":
				print("Stacking " + num + " thawing potions");
				break;
			default:
				return false;
		}
		
		if (pre) Town.goToTown();
		npc = Town.initNPC("Pots", "buyPotions");
		if (!npc) {
			if (pre) {
	...
	buyPotions: function () {
		...
		var i, j, npc, useShift, col, beltSize, pot, preAct,
			needPots = false,
			needBuffer = true,
			buffer = {
				hp: 0,
		...
		//In normal go to a4 for better pots if 120k+ and go to a3 for cheaper pots if less
		if (me.diff == 0) {
			if (me.gold > 120000) {
				if (me.act < 4 && Pather.accessToAct(4)) {
					preAct = me.act;
					this.goToTown(4);
				}
			} else {
		...
		if (!npc) {
			if (preAct) this.goToTown(preAct);
			return false;
		}
	...
	fillTome: function (code) {
		if (me.gold < 450 || (me.diff === 0 && me.gold < 1000)) {
			return false;
		}
	...
	identify: function () {
		...
		for (i = 0; i < list.length; i += 1) {
			...
			if ((!list[i].getFlag(0x10) || Config.LowGold > 0) && ([-1, 4].indexOf(Pickit.checkItem(list[i]).result) > -1 || (!list[i].getFlag(0x10) && (Item.hasTier(list[i]) || Item.hasSecondaryTier(list[i]) || Item.hasMercTier(list[i]))))) {
				break;
			}
		}
...
MainLoop:
		while (list.length > 0) {
			...
			if (!item.getFlag(0x10) && item.location === 3 && this.ignoredItemTypes.indexOf(item.itemType) === -1) {
                ...
                if (result.result === 1 && !item.getFlag(0x10) && (Item.hasTier(item) || Item.hasSecondaryTier(item) || Item.hasMercTier(item))) {
                    result.result = -1;
                }
...
CursorLoop:
	...
	shopItems: function () {
		...
		for (i = 0; i < items.length; i += 1) {
			...
			result = NTIP.CheckItem(items[i], NTIP_CheckListNoTier, true);
				...
				for (i = 0; i < items.length; i += 1) {
					...
					if (newItem) {
						...
						case 1:
							...
							if (result.line.indexOf("tier") === -1) Misc.logItem("Gambled", newItem, result.line);
	...
	invSort: function() {
		...
		if (!Storage.Inventory.CanFit(fit) && Storage.Stash.CanFit(fit)) {
...
MainLoop:
	...
	canStash: function (item) {
		...
		if ([603, 604, 605].indexOf(item.classid) !== -1 && Pickit.checkItem(item).result === 1) {
			return false
		}
	...
	stash: function (stashGold, force=false) {
		...
		var i, result, tier, sorted,
			ignoredClassids = [91, 174],
			items = Storage.Inventory.Compare(Config.Inventory);

		if (items) {
			for (i = 0; i < items.length; i += 1) {
				if (this.ignoredItemTypes.indexOf(items[i].itemType) == -1 && ignoredClassids.indexOf(items[i].classid) == -1) {
	...
	clearStash: function () {
		...
		while (items.length) {
			...
			if (Town.ignoredItemTypes.indexOf(item.itemType) > -1) {
				item.drop();
			} else if (item.classid !== 549 &&
				(Pickit.checkItem(item).result == 4 || NTIP.CheckItem(item, NTIP_CheckListNoTier, true).result == 0) &&
	...
	move: function (spot) {
		...
		while (!me.act && getTickCount() - tick < 5000) { // Wait for current me.act (cinematics?)
			delay(3);
		}
